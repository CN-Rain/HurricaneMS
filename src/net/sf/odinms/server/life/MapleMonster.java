...
import net.sf.odinms.server.life.MapleMonsterQuestInformationProvider.QuestDropEntry;
...
public class MapleMonster extends AbstractLoadedMapleLife {
            ...
            public void run() {
                synchronized (this) {
                    ...
                    if (mob.getId() == 9300061) {
                        ...
                        mob.setDropped(d);
                        mob.getMap().broadcastMessage(MaplePacketCreator.serverNotice(6, "[Sactualize] The Moon Bunny has made rice cake number " + d + "."));
                    }
	...
	public void damage(MapleCharacter from, int damage, boolean updateAttackTime) {
        ...
        if (this.hp < 1 && this.getId() == 9300061) {
            this.getMap().broadcastMessage(MaplePacketCreator.serverNotice(5, "[Notice] You have failed this quest."));
            this.getMap().killAllMonsters(false);
            this.getMap().scheduleWarp(from.getClient().getChannelServer().getMapFactory().getMap(910010300), getMap(), 500);
        }
