	...
	public synchronized void addPoints(MapleClient c, int points, long expiration, String reason) {
		...
		if (this.points.get(acc) >= AUTOBAN_POINTS) {
			...
			if (!c.getPlayer().isGM()) {
				...
				c.getSession().close();
				try {
					c.getChannelServer().getWorldInterface().broadcastMessage(null, MaplePacketCreator.serverNotice(0, "" + name + " has been disconnected by the system. (Last reason: " + reason + ")").getBytes());
