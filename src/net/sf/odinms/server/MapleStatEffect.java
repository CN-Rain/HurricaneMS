...
public class MapleStatEffect implements Serializable {
	...
	private boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary, Point pos) {
		...
		if (skill && (sourceid == 2311001 || sourceid == 9101000)) {
			ISkill sk = SkillFactory.getSkill(sourceid);
			int $range = applyfrom.getSkillLevel(sk) * 1000;

			MapleMonster mob = applyfrom.getMap().findClosestMonster(applyto.getPosition(), 30000 + $range);
			if (mob != null)
			mob.dispel();
		}
		if (overTime) {
		...
		if (summonMovementType != null && pos != null) {
			...
			if (!tosummon.canMultiSummon()) {
				...
				final MapleCharacter sc = applyfrom;
				applyfrom.addPirateSummon(tosummon);
				TimerManager.getInstance().schedule(new Runnable() {
					@Override public void run() {
						sc.getMap().broadcastMessage(MaplePacketCreator.removeSpecialMapObject(tosummon, true));
						sc.getMap().removeMapObject(tosummon);
						if (sc.hasPirateSummon(tosummon)) {
							sc.removePirateSummon(tosummon);
						}
					}
				},
				this.getDuration());
	...
	private void applyBuff(MapleCharacter applyfrom) {
		if (isPartyBuff() && (applyfrom.getParty() != null || isGmBuff())) {
			Rectangle bounds = calculateBoundingBox(applyfrom.getPosition(), applyfrom.isFacingLeft());
			List<MapleMapObject> affecteds = isGmBuff() ? applyfrom.getMap().getAllPlayer() : applyfrom.getMap().getMapObjectsInRect(bounds, Arrays.asList(MapleMapObjectType.PLAYER));
    ...
    private void applyBuffEffect(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary) {
        ...
        if (isMonsterRiding()) {
            ...
            if(sourceid == 5221006) {
				if (weapon != null) {
					if (!MapleItemInformationProvider.getInstance().isGun(weapon.getItemId())) {
						applyto.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "You must be holding a gun to get on the battleship."));
						applyfrom.getClient().getSession().write(MaplePacketCreator.enableActions());
						return;
					}
				} else {
                ...
                ridingLevel = 1932000;
				if (applyto.getBattleShipHp() <= 0) //regen the HP
