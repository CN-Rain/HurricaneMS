			...
			if (target != null) {
				...
				if (damage < 60000) {
					c.getPlayer().getMap().broadcastMessage(c.getPlayer(), 
							MaplePacketCreator.damageMonster(attackEntry.getMonsterOid(), damage), false, true);
					player.getMap().damageMonster(player, target, damage);
					player.checkMonsterAggro(target);
				} else if (!player.isGM()) {
					AutobanManager.getInstance().autoban(c, "High Summon Damage (" + damage + " to " + c.getPlayer().getMap().getMonsterByOid(attackEntry.getMonsterOid()).getId() +    ")");
				} else {
					c.getPlayer().getMap().broadcastMessage(c.getPlayer(), 
							MaplePacketCreator.damageMonster(attackEntry.getMonsterOid(), damage), false, true);
					player.getMap().damageMonster(player, target, damage);
