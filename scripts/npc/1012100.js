		...
		if (status == 0) {
			if (cm.getJob().equals(net.sf.odinms.client.MapleJob.BEGINNER)) {
				if (cm.getLevel() >= 10 && cm.getPlayer().getDex() >= 25)
					...
					cm.sendYesNo("Wow, you have reached the maximum of your potential. Do you wish to have a player NPC represent your power?");
					pnpc = 1;
			} else {
				if (cm.getLevel() >= 30 
			...
			if (pnpc == 1) {
				...
				if (success) {
					...
					cm.sendOk("There was a problem creating the NPC, either you already have one or all spots are taken.");
				}
				cm.dispose();
			} else {
