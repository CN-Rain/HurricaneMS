	...
	if (mode == -1) {
		...
		if (status == 0) {
			if (!cm.isLeader()) { // not party leader
					cm.sendNext("Please tell your #bParty Leader#k to talk to me.");
					cm.dispose();
			} else  { //party leader
				copns = cm.getPlayer().countItem(4001106);
				if (copns < 30) { cm.sendNext("You must have 30 or more coupons to clear this Party Quest."); cm.dispose(); }
				else {
					cm.sendNext("Wow, you collected me " + copns + " coupons! Congratulations! I will warp you out and give you your #bEXP#k and Rolly will give you the rewards!");
				}
			}
		} else if (status == 1) {
